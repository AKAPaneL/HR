  ...
  <div class="user-info">
    <el-row type="flex" justify="end">
      <el-tooltip content="打印个人基本信息">
        <router-link :to="`/employees/print/${userInfo.id}?type=personal`">
...
import { getPersonalInfo, saveUserDetailById, savePersonalInfo } from '@/api/employees'
...
export default {
  ...
  methods: {
    async getUserDetailById() {
      ...
      const { staffPhoto } = res
      if (staffPhoto) {
        // 回填上传组件
        this.$refs.staffAvatar.fileList = [{ url: staffPhoto }]
      }
      this.userInfo = res
    },
    async getPersonalInfo() {
      const res = await getPersonalInfo(this.userId)
      const { staffPhoto } = res
      if (staffPhoto) {
        this.$refs.staffImg.fileList = [{ url: staffPhoto }]
      }
      this.formData = res
    },
    async  saveUser() {
      const list = this.$refs.staffAvatar.fileList
      if (list.length) {
        const { url, success } = list[0]
        // 如果没有上传完
        if (!success) {
          this.$message.error('文件尚未上传完成')
          return
        }
    ...
    async savePersonal() {
      ...
      // 判断有没有上传
      if (list.length) {
        this.formData.staffPhoto = list[0].url
      }
      await savePersonalInfo(this.formData)
      // 提示用户
      this.$message.success('保存成功')
    }
  }
